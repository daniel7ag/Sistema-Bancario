...
class Cliente:
    ...
    def realizar_transacao(self, conta, transacao):
        if len(conta.historico.limite_transacoes()) >= 10:
            print("\n Limite de transações por dia excedido.")
            return
...
class Historico:
    ...
    def gerar_relatorio(self, opcao_extrato):
        if opcao_extrato == "d":
            print("\nDepósitos:")
            for transacao in self._transacoes:
                if transacao["tipo"] == "Deposito":
                    print(f"Valor: R$ {transacao['valor']:.2f} - Data: {transacao['data']}")
...
def exibir_extrato(clientes):
    ...
    [d]\tVer depósitos
    [s]\tVer saques
    [qualquer tecla]\tVer extrato completo
    => """)
